...
{ 
	...
	{ 
		...
		[Constructable]
		public GinsengSeed( int amount ) : base( 0xF27 ) //seed graphic
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
	...
	{ 
		...
		[Constructable] 
		public GinsengSeedling( Mobile sower ) : base( 0x913 )//I MODED Seedling Graphic Just planted  "dirt mound picture"
		{ 
			Movable = false; 
		...
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(GinsengCrop), plant.Sower ); //I MODED
	...
	public class GinsengCrop : BaseCrop //I MODED
	{ 
		...
		[Constructable] 
		public GinsengCrop( Mobile sower ) : base( 0x18e9 ) //I MODED Plant full size?
		{ 
			Movable = false; 
			Name = "Ginseng Plant"; //I MODED
		...
		public static void init ( GinsengCrop plant, bool full )//I MODED
		{
			plant.PickGraphic = ( 0xC7E );//I MODDED Picked Graphic
			plant.FullGraphic = ( 0x18e9 );//I MODED Full plant graphic
			...
			if ( DateTime.Now > lastpicked.AddSeconds(1) ) // 3 seconds between picking changed to 1 sec
			{
				...
				{ 
					...
					else //check skill and sower
					{
						...
						Ginseng crop = new Ginseng( pick ); //I MODED
		...
		{
			private GinsengCrop i_plant; //I MODED

			public CropTimer( GinsengCrop plant ) : base( TimeSpan.FromSeconds( 600 ), TimeSpan.FromSeconds( 15 ) )//I MODED
			{
