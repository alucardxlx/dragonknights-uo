...
{
	public class plus2spiritspeakgem : Item
	{
		[Constructable]
		public plus2spiritspeakgem() : this( 1 )
		{
		}

		[Constructable]
		public plus2spiritspeakgem( int amount ) : base( 0x1870 )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+2 SpiritSpeak Skill Gem"; 
		}

		public plus2spiritspeakgem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus2spiritspeakgem m_cOrb;
			public InternalTarget( plus2spiritspeakgem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.SpiritSpeak, 2);
                            else
                            {
