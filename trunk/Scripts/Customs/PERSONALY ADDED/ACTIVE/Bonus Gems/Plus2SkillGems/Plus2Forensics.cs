...
{
	public class plus2forensicsgem : Item
	{
		[Constructable]
		public plus2forensicsgem() : this( 1 )
		{
		}

		[Constructable]
		public plus2forensicsgem( int amount ) : base( 0x1870 )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+2 Forensics Skill Gem"; 
		}

		public plus2forensicsgem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus2forensicsgem m_cOrb;
			public InternalTarget( plus2forensicsgem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.Forensics, 2);
                            else
                            {
