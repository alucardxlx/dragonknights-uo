...
{
	public class plus1forensicsgem : Item
	{
		[Constructable]
		public plus1forensicsgem() : this( 1 )
		{
		}

		[Constructable]
		public plus1forensicsgem( int amount ) : base( 0x186E )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+1 Forensics Skill Gem"; 
		}

		public plus1forensicsgem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus1forensicsgem m_cOrb;
			public InternalTarget( plus1forensicsgem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.Forensics, 1);
                            else
                            {
