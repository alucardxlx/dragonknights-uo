...
{
	public class plus1campinggem : Item
	{
		[Constructable]
		public plus1campinggem() : this( 1 )
		{
		}

		[Constructable]
		public plus1campinggem( int amount ) : base( 0x186E )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+1 Camping Skill Gem"; 
		}

		public plus1campinggem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus1campinggem m_cOrb;
			public InternalTarget( plus1campinggem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.Camping, 1);
                            else
                            {
