...
{
	public class plus2lockpickinggem : Item
	{
		[Constructable]
		public plus2lockpickinggem() : this( 1 )
		{
		}

		[Constructable]
		public plus2lockpickinggem( int amount ) : base( 0x1870 )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+2 Lockpicking Skill Gem"; 
		}

		public plus2lockpickinggem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus2lockpickinggem m_cOrb;
			public InternalTarget( plus2lockpickinggem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.Lockpicking, 2);
                            else
                            {
