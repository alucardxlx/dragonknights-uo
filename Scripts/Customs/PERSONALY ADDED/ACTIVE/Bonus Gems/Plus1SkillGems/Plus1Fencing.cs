...
{
	public class plus1fencinggem : Item
	{
		[Constructable]
		public plus1fencinggem() : this( 1 )
		{
		}

		[Constructable]
		public plus1fencinggem( int amount ) : base( 0x186E )
		{
			Stackable = false;
			Weight = 0.1;
			Amount = amount;
			Hue = 0;
			Name = "+1 Fencing Skill Gem"; 
		}

		public plus1fencinggem( Serial serial ) : base( serial )
		{
		}
		...
		{
			private plus1fencinggem m_cOrb;
			public InternalTarget( plus1fencinggem cOrb ) :  base ( 8, false, TargetFlags.None )
			{
				m_cOrb = cOrb;
				AllowNonlocal = true;
			}
                    ...
                    if (from.CheckTargetSkill(SkillName.ItemID, targetyouselect, 0, 100))
                    {
                        if ( ((Item)targetyouselect).IsChildOf( from.Backpack ) )
                        {
                            if (targetyouselect is BaseWeapon && ((BaseWeapon)targetyouselect).SkillBonuses.Skill_1_Value < 1 )  //or <= if will let to 2 also
                                ...
                                ((BaseClothing)targetyouselect).SkillBonuses.SetValues(1, SkillName.Fencing, 1);
                            else
                            {
