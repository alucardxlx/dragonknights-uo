...
{	
	public class apiBeeHiveMainGump : Gump
	{
		...
		public apiBeeHiveMainGump( Mobile from, apiBeeHive hive ) : base( 20, 20 )
		{
			...
			//potions
			AddItem(195, 46, 3848);//195
			AddItem(195, 96, 3847);//185
			AddItem(195, 71, 3850);//190
			AddItem(195, 121, 3852);//183
			AddItem(195, 146, 3849);//186
            ...
            if (m_hive.IsDormant)
                AddItem(120, 172, 7186, 0);  //wake up!
            else
                AddItem(115, 172, 3705, 0);  //go dormant
            ...
            //dormant
            AddButton(130, 172, 212, 212, (int)Buttons.butDormant, GumpButtonType.Reply, 0);
        }
    ...
    public class SwitchDormantGump : Gump
    {
        apiBeeHive m_hive;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                if (m_hive == null || m_hive.Deleted)
                    return;

                if (m_hive.IsDormant)
                {
                    m_hive.IsDormant = false;
                    from.SendMessage("You woke up the bees.");
                }
                else
                {
                    m_hive.IsDormant = true;
                    from.SendMessage("The bees are now going to sleep, until you wake them up again.");
                }
            }
